...
vty_prompt (struct vty *vty)
{
	...
	if (vty->type == VTY_TERM||vty->type==VTY_CONSOLE) {
		hostname = host.name;
		if (!hostname)
		{
			uname (&names);
...
typedef struct __vty_auth_info__ {
	int    fail;
	int    encrypt;
	char * passwd;
	enum node_type node;
	enum node_type next_node;
	int (*success)(struct vty *vty, void * cls);
	int (*error)(struct vty *vty, void *cls) ;
	void * cls ;
	void * index;
} vtyauth_t;
int vty_auth(struct vty*vty, int encrypt, const char * passwd, int next_node ,
    int (*success)(struct vty *vty, void * cls),
    int (*error)(struct vty * vty,  void * cls),
    void *cls)
{
	vtyauth_t * ainfo = NULL ;

	if( NULL == vty||NULL == passwd ||(0 == next_node && NULL == success) ) {
		return EINVAL;
	}
	...
	ainfo = XCALLOC(MTYPE_TMP, sizeof(vtyauth_t));
	if( NULL == ainfo ) {
		return ENOMEM ;
	}

	ainfo->fail   = 0 ;
	ainfo->encrypt= encrypt;
	ainfo->cls    = cls ;
	ainfo->error  = error ;
	ainfo->success= success;
	ainfo->passwd = XSTRDUP(MTYPE_TMP, passwd);
	ainfo->next_node = next_node;
	ainfo->index  = vty->index ;
	ainfo->node   = vty->node ;
...
static void
__do_vty_auth (struct vty *vty, char *buf)
{
	extern char *crypt (const char *, const char *);

	int fail ;
	vtyauth_t * ainfo = vty->index;
	...
	if( ainfo->encrypt ) {
		fail = strcmp (crypt(buf, ainfo->passwd), ainfo->passwd);
	} else {
		fail = strcmp (buf, ainfo->passwd);
	}

	if( fail ) {
		ainfo->fail ++ ;

		// return the status before auth mode 
		if( ainfo->fail >= 3) {
			vty_out (vty, "%% Bad passwords, too many failures!%s", VTY_NEWLINE);
			
			vty->index = ainfo->index;
			vty->node  = ainfo->node ;

			if( ainfo->error ) {
				ainfo->error(vty, ainfo->cls) ;
			}

			XFREE(MTYPE_TMP, ainfo);
		}
	} else {
		// return first
		vty->index = ainfo->index;
		...
		// if have(next_node) 
		if( ainfo->next_node > 0 ) {
			vty->node = ainfo->next_node ;
		}

		// if have(success)
		if( ainfo->success ) {
			ainfo->success(vty, ainfo->cls) ;
		}
...
vty_command (struct vty *vty, char *buf)
{
...
#ifdef CONSUMED_TIME_CHECK
  {
    ...
    GETRUSAGE(&before);
	//printf("\nbefore: %lu.%06lu\n", before.real.tv_sec, before.real.tv_usec); fflush(stdout);
  ...
  ret = cmd_execute_command (vline, vty, NULL, 0);
	...
	//printf("\nafter: %lu.%06lu, realtime = %lu\n", after.real.tv_sec, after.real.tv_usec, realtime); fflush(stdout);
  }
#endif /* CONSUMED_TIME_CHECK */
...
vty_read (struct thread *thread)
{
  ...
  if ((nbytes = read (vty->fd, buf, VTY_READ_BUFSIZ)) <= 0)
    ...
    // PRINT("VTY_READ: %d: %.*s", nbytes, nbytes, buf);
...
vty_hidden (int fd)
{
  ...
  vty->hp = 0;
  vty->hindex = 0;
  vector_set_index (vtyvec, fd, vty);
  vty->status = VTY_NORMAL;
  vty->v_timeout = 0;
  vty->lines = -1;
  vty->iac = 0;
  ...
  vty_event (VTY_WRITE, fd, vty);
  vty_event (VTY_READ, fd, vty);
...
/* vty_create */
struct vty *
vty_console (const char * str, int first_node)
{
  ...
  /* init tty socket */
  tty = open(str, O_RDWR|O_NONBLOCK) ;
  if( tty < 0 ) {
  	return NULL ;
  }
  tcgetattr(tty, &t) ;
  ...
  memcpy (&vty->orig, &orig, sizeof(orig));
  ...
  vty->hp = 0;
  vty->hindex = 0;
  vector_set_index (vtyvec, tty, vty);
  vty->status = VTY_NORMAL;
  ...
  /* Add read/write thread. */
  vty_event (VTY_WRITE, tty, vty);
  vty_event (VTY_READ, tty, vty);
...
vty_close (struct vty *vty)
{
  ...
  THREAD_TIMER_OFF(vty->t_timeout);
  ...
  /* Close socket. */
  if (vty->fd > 0 ) {
  	/* reset TTY attribute for console */
    if( type == VTY_CONSOLE ) {
      tcsetattr(vty->fd, TCSANOW, &vty->orig) ;
    }
    close (vty->fd);
  }
...
vty_timeout (struct thread *thread)
{
  ...
  /* Clear buffer*/
  buffer_reset (vty->obuf);
  vty_out (vty, "%sConsole session is timed out.%s", VTY_NEWLINE, VTY_NEWLINE);
...
vty_event (enum event event, int sock, struct vty *vty)
{
  ...
  switch (event)
    ...
    case VTY_SERV:
      ...
      break;
    case VTY_READ:
      THREAD_READ_ON(master, vty->t_read, vty_read, vty, sock);
      if (vty->v_timeout)
	{
      THREAD_TIMER_OFF(vty->t_timeout) ;
      THREAD_TIMER_ON(master, vty->t_timeout, vty_timeout, vty, vty->v_timeout);
	}
      break;
    case VTY_WRITE:
      THREAD_WRITE_ON(master, vty->t_write, vty_flush, vty, sock) ;
      break;
...
vty_init (struct thread_master *master_thread)
{
  ...
  if( 0 ) {
	  ...
	  install_element (ENABLE_NODE, &exec_timeout_min_cmd);
