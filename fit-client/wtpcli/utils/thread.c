...

#if 0 // &&  !defined(HAVE_CLOCK_MONOTONIC) && !defined(__APPLE__)
static void
...
#elif 0 //  /* !HAVE_CLOCK_MONOTONIC && !__APPLE__ */
  ...
  if (realtime > CONSUMED_TIME_CHECK)
    {
       ...
       * to fix.
       */
      zlog_warn ("SLOW THREAD: task %s (%lx) ran for %lums (cpu time %lums)[ARG:%p, VAL:%d]",
		 thread->funcname,
		 (unsigned long) thread->func,
		 realtime/1000, cputime/1000, THREAD_ARG(thread), THREAD_VAL(thread));
    }
