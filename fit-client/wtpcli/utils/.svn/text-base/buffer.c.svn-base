...
int buffer_get(struct buffer *b, char * buf, size_t bufsiz)
{
	int gsiz = 0 ;
	int once = 0 ;
	int valid= 0 ;
	...
	while( gsiz < bufsiz && NULL != (d=b->head) ) {
		once = bufsiz - gsiz ; 
		valid= d->cp  - d->sp ;
		...
		memcpy(buf+gsiz, d->data+d->sp, once) ;
		d->sp += once ;
		gsiz  += once ;

		if( d->cp <= d->sp ) {
			if( NULL ==(b->head = d->next) ) {
				b->tail = NULL ;
			}
			BUFFER_DATA_FREE(d) ;
		}
...
int buffer_get_at(struct buffer *b, size_t off, char *buf, size_t max)
{
	int gsiz = 0 ;
	int once = 0;
	int valid= 0;
	...
	for(d=b->head; d && gsiz < max; d=d->next) {
		valid = d->cp - d->sp ;
		osp  = nsp ;
		nsp += valid ;
		if( nsp <= off ) {
			/* PRINT("DATA %p, OSP %ld, NSP %ld, VALID %d",
			   d, osp, nsp, valid); */
			continue ;
		}

		base  = (off > osp)? off - osp : 0 ;
		valid-= base ;
		once  = max - gsiz ;
		if( once > valid ){
			once = valid ;
		}
		/* PRINT("DATA %p, OSP %ld, NSP %ld, VALID %d, BASE %d, ONCE %d", 
			d, osp, nsp, valid, base, once); */
		memcpy(buf+gsiz, d->data+d->sp+base, once) ;
		gsiz += once ;
...
int 
buffer_write_file(struct buffer *b, 
	int (*write_func)(void *file, void *data, int size), 
	void *file )
{
	int wlen; 
	int dlen;
	...
	while( NULL != (d= b->head) ) {
		dlen = (d->cp-d->sp) ;
		wlen = write_func(file, d->data + d->sp, dlen) ;
		if( wlen == dlen ) {
			if (!(b->head = d->next))
        		b->tail = NULL;
        	BUFFER_DATA_FREE(d);
...
int 
buffer_gzip(struct buffer *b)
{
	return 0 ;
}
int 
buffer_ungzip(struct buffer *b)
{
	return 0 ;
}
