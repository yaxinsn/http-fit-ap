...
"\0\r\n\rWelcome to Command Line Interface.\r\n\r\n";
...
static enum match_type
cmd_mac_address_match (const char *str)
{
	unsigned int n ;
	int len ;
	char sep ;
	...
	if( len <= 2 || isxdigit((int)str[2]) ) {
		if( len > 12 ) return no_match ;
		for( n=0; n<len; n++) {
			if( !isxdigit((int)str[n]) ){
				return no_match ;
			}
		}
		return ( len < 12 ) ? partly_match : exact_match;
	}

	/* have seprator */
	if( len > 17 ) return no_match ;
	sep = str[2] ;
	for(n=0; n<len; n++) {
		if( (n%3)==2 && str[n] == sep )
			continue ;
	...
	return ( len < 17 ) ? partly_match : exact_match;
}
	...
	else
	  {
	      ...
	      if ((desc = vector_slot (descvec, j)))
		{
		  ...
		  if (CMD_VARARG (str))
		    ...
		    {
		    	if( cmd_mac_address_match(command)) {
	...
	for (j = 0; j < vector_active (descvec); j++)
	  if ((desc = vector_slot (descvec, j)))
		...
		case ipv4_prefix_match:
		  ...
		  break;
		case mac_match: 
		  if((ret = cmd_mac_address_match(command)) != no_match){
		  	if( ret == partly_match )
		  		return 2 ;
		  	match++ ;
		  }
...
cmd_entry_function (const char *src, const char *dst)
{
  ...
  if (CMD_OPTION (dst) || CMD_VARIABLE (dst) || CMD_VARARG (dst) ||
      CMD_IPV4 (dst) || CMD_IPV4_PREFIX (dst) || CMD_RANGE (dst) || CMD_MAC(dst) ) {
    /* todo: tab completed */
	...
	if (match == vararg_match)
	  {
	    ...
	    for (j = 0; j < vector_active (cmd_vector); j++)
		  ...
		  && (vector_active (cmd_element->strvec)))
		{
		  if( cmd_element->attr == CMD_ATTR_HIDDEN ) continue ;
		  ...
		  if ((desc = vector_slot (descvec, j)))
		    {
		      ...
		      /* todo: '?' for describe command */
		      if( cmd_element->describe && 
		         1 == cmd_element->describe(cmd_element, desc, matchvec, command) ) {
		         continue ;
		      }
  ...
  for (i = 0; i < vector_active (cmd_vector); i++)
	...
	if (index >= vector_active (strvec))
	...
	else
	  {
		...
		{
		  /* todo: '\t' complete */
		  if( cmd_element->complete && 
		      1 ==  cmd_element->complete(cmd_element, desc, matchvec, vector_slot (vline, index)) ) {
		    continue ;
		  }
